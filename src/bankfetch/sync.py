    ...
    *,
    ...
    with FileLock(store.lock_dir):
        ...
        with enable_client(config, jwt_token) as client:
            ...
            logger.info("sync_started", extra={"event": "sync_started", "account_count": len(accounts), "dry_run": dry_run})
            for account in accounts:
                try:
                    ...
                    for page in client.iter_transactions(
                        ...
                        if not dry_run:
                            store.archive_raw("transactions", account.account_key, utc_now(), page, page_number=page_index)
                    latest_records = dedupe_transactions(store.read_latest_transactions(account.account_key) + transaction_records)
                    if not dry_run:
                    ...
                    summary.failed_accounts.append(account.account_key)
                    logger.exception(
                        "account_sync_failed",
                        extra={"event": "account_sync_failed", "account_key": account.account_key},
                    )
                    if fail_fast:
                        raise
            if not dry_run:
